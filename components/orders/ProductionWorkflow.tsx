...
import { 
  ...
  ChevronDown, ChevronUp, Download, Timer, AlertCircle, Check, RotateCcw
} from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pb-20 no-scrollbar relative">
      ...
      <div className="space-y-4 no-print">
        <StageCard title="Fabric Procurement" icon={<Truck size={20}/>} stage={stages[1]} expanded={expandedStage === 1} onToggle={() => setExpandedStage(expandedStage === 1 ? null : 1)}>
          <div className="space-y-6">
            {!stages[1].mode ? (
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-black text-gray-400 uppercase block mb-1">Set Start Date</label>
                  <input type="date" min={minAllowedDate} className="p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold text-xs w-full max-w-xs" value={stages[1].startDate ? new Date(stages[1].startDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]} onChange={(e) => handleStageUpdate(1, { startDate: new Date(e.target.value).toISOString() })} />
                </div>
              ...
              <div className="space-y-6">
                <div className="flex items-end gap-4">
                  ...
                  <div><label className="text-[10px] font-black text-gray-400 uppercase block mb-1">Manual Start Date</label><input type="date" min={minAllowedDate} className="p-3 bg-gray-50 border border-gray-200 rounded-xl font-black text-sm" value={stages[1].startDate ? new Date(stages[1].startDate).toISOString().split('T')[0] : ''} onChange={(e) => handleStageUpdate(1, { startDate: new Date(e.target.value).toISOString() })} /></div>
                </div>
                ...
                <div className="flex justify-between border-t pt-4 items-end">
                    <button onClick={() => handleReset(1)} disabled={!canReset(1)} className={`flex items-center gap-2 text-[10px] uppercase font-black underline mb-2 ${!canReset(1) ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-red-500'}`}><RotateCcw size={12}/> Reset Stage 1</button>
                      ...
                      {stages[1].mode === 'vendor' && !stages[1].poConfirmed ? (
                        ...
                        <div className="flex flex-col gap-2 items-end">
                          ...
                          <div className="flex gap-2">
                            <input type="date" min={stages[1].startDate ? new Date(stages[1].startDate).toISOString().split('T')[0] : minAllowedDate} className="p-2 border border-gray-200 rounded-lg text-xs font-bold" value={stages[1].actualDate ? new Date(stages[1].actualDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]} onChange={(e) => handleStageUpdate(1, { actualDate: new Date(e.target.value).toISOString() })} />
                            {stages[1].mode === 'vendor' && <button onClick={() => triggerPrint(1, 'Fabric Procurement PO')} className="px-6 py-3 border-2 border-blue-600 text-blue-600 rounded-xl font-black text-[10px] uppercase flex items-center gap-2"><Download size={14}/> Print PO</button>}
          ...
          <StageCard key={s.id} title={s.name} icon={s.icon} stage={stages[s.id]} expanded={expandedStage === s.id} onToggle={() => !isLocked(s.id) && setExpandedStage(expandedStage === s.id ? null : s.id)} locked={isLocked(s.id)}>
              <div className="space-y-6">
                <div className="flex items-end gap-4">
                  ...
                  <div><label className="text-[10px] font-black text-gray-400 uppercase block mb-1">Manual Start Date</label><input type="date" min={minAllowedDate} className="p-3 bg-gray-50 border border-gray-200 rounded-xl font-black text-sm" value={stages[s.id].startDate ? new Date(stages[s.id].startDate).toISOString().split('T')[0] : ''} onChange={(e) => handleStageUpdate(s.id, { startDate: new Date(e.target.value).toISOString() })} /></div>
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-end pt-4">
                        ...
                        <div className="flex flex-col gap-2 items-end">
                          <label className="text-[10px] font-black text-gray-400 uppercase">Manual Completion Date</label>
                          <div className="flex gap-2">
                            <input type="date" min={stages[s.id].startDate ? new Date(stages[s.id].startDate).toISOString().split('T')[0] : minAllowedDate} className="p-2 border border-gray-200 rounded-lg text-xs font-bold" value={stages[s.id].actualDate ? new Date(stages[s.id].actualDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]} onChange={(e) => handleStageUpdate(s.id, { actualDate: new Date(e.target.value).toISOString() })} />
        ...
        <StageCard title="Pattern & Sampling" icon={<Scissors size={20}/>} stage={stages[5]} expanded={expandedStage === 5} onToggle={() => !isLocked(5) && setExpandedStage(expandedStage === 5 ? null : 5)} locked={isLocked(5)}>
          <div className="space-y-6">
            <div className="flex items-end gap-4">
              ...
              <div><label className="text-[10px] font-black text-gray-400 uppercase block mb-1">Manual Start Date</label><input type="date" min={minAllowedDate} className="p-3 bg-gray-50 border border-gray-200 rounded-xl font-black text-sm" value={stages[5].startDate ? new Date(stages[5].startDate).toISOString().split('T')[0] : ''} onChange={(e) => handleStageUpdate(5, { startDate: new Date(e.target.value).toISOString() })} /></div>
            </div>
            {stages[5].status === 'pending' ? (
              <div className="flex gap-4">
                <button onClick={() => handleStageUpdate(5, { status: 'na' })} className="flex-1 py-5 border-2 text-gray-400 border-gray-100 rounded-2xl font-black uppercase text-xs">Not Required</button>
                <button onClick={() => handleStageUpdate(5, { status: 'in_progress', startDate: stages[5].startDate || new Date().toISOString() })} className="flex-1 py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs shadow-lg">Start Stage</button>
              </div>
            ) : (
              <>
                <table className="w-full text-left text-xs">
                ...
                <div className="flex flex-col gap-4 pt-4 border-t">
                  <div className="flex flex-col gap-2 items-end">
                    ...
                    <div className="flex gap-2 w-full">
                      <input type="date" min={stages[5].startDate ? new Date(stages[5].startDate).toISOString().split('T')[0] : minAllowedDate} className="flex-1 p-3 border border-gray-200 rounded-xl text-sm font-bold" value={stages[5].actualDate ? new Date(stages[5].actualDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]} onChange={(e) => handleStageUpdate(5, { actualDate: new Date(e.target.value).toISOString() })} />
            ...
            <button onClick={() => handleReset(5)} disabled={!canReset(5)} className={`flex items-center justify-center gap-2 text-[10px] uppercase font-black underline p-2 mt-2 ${!canReset(5) ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-red-600'}`}><RotateCcw size={12}/> Reset Sampling Stage</button>
          </div>
        </StageCard>
      </div>
      ...
      <div className="hidden print:block p-10 bg-white">
          ...
          <div className="mt-10 p-4 border-2 border-black"><p className="text-sm font-bold uppercase tracking-widest">Internal Production Sheet - Authorized Use Only</p></div>
      </div>
    </div>
  );
}
          ...
          <div>
            ...
            {!locked && (
              <div className="flex gap-4 mt-1 items-center">
                <span className={`text-[10px] font-black uppercase ${stage.status === 'completed' ? 'text-emerald-500' : 'text-blue-600'}`}>{stage.status === 'na' ? 'Not Required' : stage.status.replace(/_/g, ' ')}</span>
